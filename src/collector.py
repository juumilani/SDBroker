...
class Collector:
    ...
    def __init__(self, temp, nsala, port=4321, max_reconnects=5):
    ...
    def run(self):
        ...
        while True:
            ...
            # Regular workflow
                ...
                # Send current temp rate and calculate a new nsala
                else:
                    self.logger.info('Sending current rates to the broker...')
                    rate = b'{%b} %.2f' % (self.temp.encode('utf-8'), self.nsala)
                    self.socket.send(rate)
                    self.nsala = (self.nsala + uniform(-(self.nsala * 0.2), self.nsala * 0.2))
    ...
    def start(self):
        ...
        self.socket.setsockopt(zmq.IDENTITY, bytes(self.temp.encode('utf-8')))
    ...
    def set_logger(self):
        self.logger = logging.getLogger(self.temp)
...
def main(argv):
    worker = Collector(argv[0], float(argv[1]))
    worker.run()
